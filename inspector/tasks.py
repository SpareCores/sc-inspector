...
class StressNgLongRun(lib.DockerTask):
    ...
    version_docker_opts: dict = dict(entrypoint="sh")
    version_command: str = "-c \"stress-ng --version | awk '{print $3}'\""
    command: str = '-c "nice -n -20 for i in {1..5}; do SPM=$(($(($i / 60 + 1)) * 5)); SPM=$(( $SPM > 55 ? 55 : $SPM )); stress-ng --metrics --cpu $(nproc) --cpu-method div16 -t $(( $SPM > 55 ? 55 : $SPM )) -Y /dev/stderr; sleep $((60 - $(date +%-S.%N) )); done"'
